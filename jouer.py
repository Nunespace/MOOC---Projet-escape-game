...
import time
turtle2=Turtle() #création d'une 2ème tortue pour affichage des annonces et de l'inventaire
...
def texte_intro():
    ...
    time.sleep(1)
    goto(-325,100)
    write ("Grizzy est à la pêche. Super, ça mord! Il sort un énorme poisson de l'eau.", font=("Comic sans ms",11, "bold"))
    goto(-325,80)
    write ("Il se lèche les babines à l'idée de le manger avec du Nutella tartiné dessus.",font=("Comic sans ms",11, "bold"))
    goto(-325,60)
    write ("Il se précipite au coeur de la forêt pour inviter sa copine à manger.",font=("Comic sans ms",11, "bold"))
    goto(-325,40)
    write ("Les Lemmings cachés en colonne derrière un arbre, en profitent pour lui voler son poisson",font=("Comic sans ms",11, "bold"))
    goto(-325,20)
    write ("et le cacher dans un labyrinthe magique.",font=("Comic sans ms",11, "bold") )
    goto(-325,0)
    write ("A son retour, Grizzy est horrifié quand il découvre que le poisson a disparu.", font=("Comic sans ms",11, "bold"))
    goto(-325,-40)
    write ("Tout à coup, il aperçoit les Lemmings sortir en courant et en ricanant du labyrinthe magique.",font=("Comic sans ms",11, "bold") )
    goto(-325,-60)
    write ("Grizzy les poursuit. Il court, il court.. Fatigué, il s'arrêté et s'appuie contre un rocher.",font=("Comic sans ms",11, "bold") )
    goto(-325,-80)
    write ("Une grotte s'ouvre! Il rentre et trouve un parchemin magique : le plan du labyrinthe!.", font=("Comic sans ms",11, "bold"))
    goto(-325,-100)
    write ("Aide Grizzy à retouver son poisson à l'aide de ce plan.", font=("Comic sans ms",11, "bold"))
    goto(100,-140)
    color ('green')
...
def calculer_pas(matrice): # calcul de la longueur d'une case en px selon taille de la matrice(plan)
...
def coordonnes (case,pas): # calcul des coordonnées x,y à partir des couples de la matrice (case du plan)
    x1=ZONE_PLAN_MINI[0]
    y1=ZONE_PLAN_MINI[1]
    x=case[1]*pas+x1
...
def afficher_plan(matrice): # fonction3 (principale) pour dessiner le plan selon les paramètres (CONFIGS.py)
    hideturtle()
...
def creer_dictionnaire_des_objets(fichier_des_objets): # objets = indices ici
    ...
    for m in open(fichier_des_objets, encoding="utf-8"):
        dico_objets.setdefault(eval(m)[0], eval(m)[1]) # la fonction eval accepte la chaîne de caractère m et retourne un objet
    return dico_objets
    
def creer_dictionnaire_des_portes(fichier_des_portes): # fichier questions/réponses pour ouvrir les portes
    dico_portes={}
...
def deplacer_gauche(): #touche Left du clavier 
    ...
    onkeypress(deplacer_gauche, "Left")   # Réassocie la touche Left à la fonction deplacer_gauche
    if position [1]>0:
...
def deplacer_droite(): #touche Right du clavier
    global matrice, position
    onkeypress(None, "Right")   # traitement associé à la flèche droite appuyée par le joueur
...
def deplacer (matrice, position, mouvement): #déplacement dans les couloirs (cases vides blanches)
    speed(4)
...
def ramasser_objet (matrice, position, mouvement): #case verte (indices)
    ...
    turtle2.color('green')
    turtle2.write("Bravo! Tu as trouvé un indice : "+ indice, font=("Comic sans ms",14, "normal"))
...
def maj_inventaire (indice, mouvement): # pour chaque indice ramassé, l'inventaire est mis à jour et réécrit en bas à droite
...
def poser_question(matrice,position, mouvement): # cases oranges (portes)
    ...
    while True:
        ...
        if reponse!=portes[mouvement][1]:
            turtle2.color('red')
            turtle2.write("Mauvaise réponse. La porte reste fermée!", font=("Comic sans ms",15, "bold"))
            time.sleep(2)
            turtle2.undo()
        elif reponse==portes[mouvement][1]:
            turtle2.color('green')
            turtle2.write("Bravo! La porte est ouverte!", font=("Comic sans ms",15, "bold"))
            time.sleep(1)
            turtle2.undo()
            return ouverture_porte (matrice,position,mouvement)
    
        
def ouverture_porte (matrice,position,mouvement): #en cas de bonne réponse, la porte s'ouvre ainsi
    tracer_case (position, "white", pas)
    tracer_case (mouvement, "white", pas)
    up()
    x, y= coordonnes (mouvement, pas)
...
def poser_question_finale(matrice,position, mouvement): #case jaune (sortie/objectif : le poisson!)
    ...
    turtle2.down()
    turtle2.color('red')
    turtle2.write("Ca sent le poisson derrière cette porte...!", font=("Comic sans ms",15, "normal"))
    time.sleep(3)
    turtle2.undo()
    ...
    while True:
        ...
        if reponse == portes[mouvement][1] or reponse==portes[mouvement][2]: #2ème réponse avec majuscule
            return the_end()
        elif reponse == portes[mouvement][3] or reponse==portes[mouvement][4]: # 2 réponses avec erreur "classique" d'orthographe
            turtle2.write("Es-tu sûr que ça s'écrit comme ça?", font=("Comic sans ms",16, "bold"))
            #time.sleep(2)
            #turtle2.undo()
        else:
            turtle2.write("Réfléchie encore... Grizzy veut manger avec son amie!", font=("Comic sans ms",16, "bold"))
            time.sleep(2)
            turtle2.undo()
        
                    

def demarrer_jeu(a,b): #2ème fenêtre après l'intro : le jeu lui-même (affichage nouveau fond, plan, indication pour le joueur...)    
    clearscreen()
    ...
    turtle2.color("green")
    turtle2.goto (x_a, y_a)
    turtle2.write("Déplace Grizzy avec les flèches de ton clavier.", font=("Comic sans ms",15, "normal"))
    time.sleep(3)
    turtle2.undo()
    up()
...
def jouer(): #musique d'ambiance, associations des flèches du clavier aux fonctions déplacer_...
    ...
    onkeypress(deplacer_droite, "Right")# Associe à la touche Left une fonction appelée deplacer_droite
    onkeypress(deplacer_haut, "Up")# Associe à la touche Left une fonction appelée deplacer_haut
    onkeypress(deplacer_bas, "Down")# Associe à la touche Left une fonction appelée deplacer_bas
...
def the_end(): # phrase de félicitations puis nouvelle fenêtre de Fin
    turtle2.color('green')
    turtle2.write("FELICITATIONS! Grizzy va enfin pouvoir manger!", font=("comic sans ms",16, "bold"))
    time.sleep(3)
    turtle2.undo()
    clearscreen()
    up()
